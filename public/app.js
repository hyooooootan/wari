...
function renderHouseholdCalendar() {
  ...
  for (let day = 1; day <= days; day += 1) {
    ...
    const total = rows.filter((transaction) => transaction.status !== "provisional").reduce((sum, transaction) => sum + integer(transaction.paid_amount), 0);
  ...
  const monthTotal = monthTransactions.filter((transaction) => transaction.status !== "provisional").reduce((sum, transaction) => sum + integer(transaction.paid_amount), 0);
