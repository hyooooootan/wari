...
`db/schema.sql` は空の D1 に七表を作成します。`db/migrations/0001_initial.sql` は初期七表、`db/migrations/0002_household_ledger.sql` は旧七表が存在する D1 を現在の七表へ移行します。番号付き移行は Wrangler の移行履歴で管理します。
