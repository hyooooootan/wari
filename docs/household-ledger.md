...
レシート、Gmail支払い通知、解析済み通知は、取引を直接上書きせず `import_records` に保存します。取込元の情報、解析結果、接続、確認、除外、失敗の状態を保持します。既存のCSV取込記録は引き続き表示・照合でき、CSV APIも互換性のため維持します。画面からのCSV追加は提供しません。
...
同点または近い点数の候補がある場合は自動接続しません。利用者が確認した値を項目ごとに優先し、確定済みの取込値を通知で上書きしません。
