...
- A1への訂正学習版コード反映
- A1へのOllama導入とモデル取得
- A1上のA・B・C・D比較、p50、p95、最大RSS、同時要求測定
- 本番Pagesの機能設定変更

この作業環境ではSSH秘密鍵の読取りとA1への通信が拒否されたため、実機反映は行っていません。基点版の既存測定は2 OCPU、約12GB、通常経路約11.8秒から12.4秒です。新しい測定値は`docs/receipt-ocr-feedback.md`の評価手順で取得します。
